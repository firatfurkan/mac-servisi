...
export default function DateStrip() {
  ...
  function prevPage() {
    setPageStart((prev) => dayjs(prev).subtract(DAYS_VISIBLE, 'day').format('YYYY-MM-DD'));
  }

  function nextPage() {
    setPageStart((prev) => dayjs(prev).add(DAYS_VISIBLE, 'day').format('YYYY-MM-DD'));
  }
  ...
  return (
    <View style={[styles.wrapper, { backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.divider }]}>
      <View style={styles.row}>
        {/* Prev */}
        <TouchableOpacity onPress={prevPage} activeOpacity={0.6} style={styles.arrowBtn}>
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  isTodayDate && !isSelected && { backgroundColor: theme.colors.primary + '22' },
                ]}
                activeOpacity={0.7}
              >
                <Text style={[
                  styles.dayLabel,
                  { color: isSelected ? '#fff' : isTodayDate ? theme.colors.primary : theme.colors.textSecondary },
                ]}>
                  {top}
                </Text>
                <Text style={[
                  styles.dateLabel,
                  { color: isSelected ? '#fff' : isTodayDate ? theme.colors.primary : theme.colors.textPrimary },
        ...
        {/* Next */}
        <TouchableOpacity onPress={nextPage} activeOpacity={0.6} style={styles.arrowBtn}>
        ...
        {/* Calendar icon */}
        <TouchableOpacity onPress={openCalendar} activeOpacity={0.6} style={styles.calendarBtn}>
      ...
      {!isOnCurrentPage && (
        <TouchableOpacity onPress={goToday} activeOpacity={0.7} style={[styles.todayBtn, { borderColor: theme.colors.primary }]}>
          ...
          <Pressable style={styles.modalOverlay} onPress={() => setShowCalendar(false)}>
            <Pressable style={[styles.calendarModal, { backgroundColor: theme.colors.surface }]} onPress={(e) => e.stopPropagation()}>
              <View style={styles.calendarHeader}>
                <TouchableOpacity onPress={() => setCalendarMonth(prev => prev.subtract(1, 'month'))}>
                ...
                <TouchableOpacity onPress={() => setCalendarMonth(prev => prev.add(1, 'month'))}>
              ...
              <View style={styles.calendarWeekRow}>
                {Array.from({ length: 7 }, (_, i) =>
                  dayjs().day((i + 1) % 7).locale(i18n.language).format('dd')
                ).map((d, i) => (
                  <Text key={i} style={[styles.calendarWeekDay, { color: theme.colors.textSecondary }]}>{d}</Text>
                ))}
              </View>
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          isTodayDay && !isSelected && { backgroundColor: theme.colors.primary + '22', borderRadius: 20 },
                        ]}
                      ...
                      >
                        <Text style={[
                          ...
                          { color: isSelected ? '#fff' : isTodayDay ? theme.colors.primary : theme.colors.textPrimary },
              ...
              <TouchableOpacity
                ...
                style={[styles.calendarTodayBtn, { backgroundColor: theme.colors.primary }]}
              >
                <Text style={styles.calendarTodayBtnText}>{t('home.today')}</Text>
              </TouchableOpacity>
            </Pressable>
          </Pressable>
        </Modal>
